...
Ядро остаётся анализатором схемы. Отдельный пакет `generate.s1ap` реализует
селективный генератор Java IE для `naseps`: [инструкция и примеры](GENERATOR_README.md).
