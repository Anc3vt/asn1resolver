...
public final class GtpTeid implements InformationElement {
    private static final AperRange RANGE = new AperRange("4:4");
    private final byte[] bytes;

    public GtpTeid(BitInput in) {
        try {
            this.bytes = aperOctets(in, RANGE, false, false);
    ...
    public GtpTeid(byte[] bytes) {
        ...
        aperSize(bytes.length, RANGE, false, false);
    ...
    public void encode(BitOutput out) {
        aperOctets(out, bytes, RANGE, false, false);
    }
